...
@Mapper (componentModel = "spring", unmappedSourcePolicy = ReportingPolicy.WARN, unmappedTargetPolicy =
...
public interface FirebaseMapper {
    ...
    @Mapping (target = "photoUrl", source = "image.url")
    ...
    @Mapping (target = "photoUrl", source = "request.image.url")
    @Mapping (target = "email", source = "request.email")
    @Mapping (target = "phoneNumber", source = "request.phoneNumber")
//    @Mapping (target = "email", source = "request.email", conditionExpression = "java(request.email() != null && " +
//                                                                                "!request.email().equalsIgnoreCase(user.getEmail()))")
//    @Mapping (target = "phoneNumber", source = "request.phoneNumber", conditionExpression = "java(request.phoneNumber()!=null && !request.phoneNumber().equals(user.getPhoneNumber()))")
